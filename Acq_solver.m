...
switch S.acqui.solver
    case 'fmincon'
        ...
        S.acqui.x = fmincon(@(x) -Acqfn(x,S), zeros(S.prob.dim,1),[],[],[],[],S.prob.min*ones(1,S.prob.dim),S.prob.max*ones(1,dim),[],options)';
    ...
    case 'pso'
        options = optimoptions('particleswarm','UseParallel',true,'SwarmSize',200,'UseParallel',true);
