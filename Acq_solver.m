...
switch S.acqui.solver
    case 'ga'
        ...
        S.acqui.x = ga(@(x) -Acqfn(x,S),S.prob.dim,[],[],[],[],S.prob.min*ones(S.prob.dim,1),S.prob.max*ones(S.prob.dim,1),[],options);       % x point to make EI maximum
    ...
    case 'fmincon'   % only difference is method to find mininum value
        ...
        S.acqui.x = fmincon(@(x) -Acqfn(x,S),zeros(S.prob.dim,1),[],[],[],[],S.prob.min*ones(1,S.prob.dim),S.prob.max*ones(1,dim),[],options)';   % different part
