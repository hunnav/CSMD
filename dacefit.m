...
% Check design points
[m,n] = size(S);  % m is number n is dimension (design sites)
sY = size(Y);   % sY = m (maybe)
if  min(sY) == 1 % single object function
    Y = Y(:);
    lY = max(sY); % lY is the number of the y (or length) = m
else
    lY = sY(1);
end
if m ~= lY % if number of x and y is different
    error('S and Y must have the same number of rows')
end

% Check correlation parameters
lth = length(theta0);
if  nargin > 5  % optimization case  >> if lob and upb is exist
...
% Normalize data
mS = mean(S);   sS = std(S);   % in the same column
mY = mean(Y);   sY = std(Y);
...
S = (S - repmat(mS,m,1))./ repmat(sS,m,1);   % 이걸로 바꿔버리네
Y = (Y - repmat(mY,m,1))./ repmat(sY,m,1);   % 이걸로 바꿔버리네
...
% Regression matrix
F = feval(regr, S);  [mF,p] = size(F);  % p is the number of f
if  mF ~= m
...
par = struct('corr',corr, 'regr',regr, 'y',Y, 'F',F, 'D', D, 'ij',ij, ...
    'scS',sS);
